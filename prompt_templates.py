...
## 2. source language 데이터
{source_sentences}

## 3. target language 데이터
{target_sentences}

# 작업 규칙
1. corpus에는 단어의 '원형'을 포함할 것
2. corpus 속 단어들에 rule을 적절히 적용하면 예시 data가 완벽히 설명되도록 만들 것
3. 데이터가 확고한 규칙을 설정하기엔 부족하다면 보류할 것
4. corpus에는 조사, 전치사 등도 반드시 포함할 것. 그래서 추후 해석할 때 단어를 모르는 경우가 절대로 발생하지 않도록 할 것.
5. 해석이 불확실한 단어는 unknown: 단어1, 단어2, ... 형식으로 적을 것
...
3. 데이터가 확고한 규칙을 설정하기엔 부족하다면 보류할 것
4. corpus에는 조사, 전치사 등도 반드시 포함할 것. 그래서 추후 해석할 때 단어를 모르는 경우가 절대로 발생하지 않도록 할 것
5. 도출한 rule을 바탕으로 지금까지의 unknown 단어의 의미를 찾을 것
6. 지금까지 도출된 rule을 바탕으로 해석할 수 없는 단어는 unknown: 단어1, 단어2, ... 형식으로 적을 것
